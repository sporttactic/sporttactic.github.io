...
Views.dashboard = function (mount) {
  ...
  const matches = Store.matches().slice().sort((a, b) => a.date - b.date);
  const now = Date.now();
  ...
  mount.innerHTML = `
    ...
    <div class="page-head">
      ...
      <button class="btn primary" data-route-link="scouting" ${Access.moduleOpen('scouting') ? '' : 'data-write'}>${T('dash.startScouting')}</button>
    </div>
    ${UI.acc('dashStats', T('dash.season'), stats)}
    ${UI.acc('dashMatches', T('dash.matchesArea'), matchCards)}
    ${UI.acc('dashSquad', T('dash.squadArea'), squadCards)}`;
